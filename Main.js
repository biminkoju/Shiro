...
const { GetDate, GetTime } = require('./api/functions');
...
client.on('message', async message => {
	...
	let CommandArg = message.content.toLowerCase().slice(Prefix.length).trim().split(/ +/g);
	let commandfile = client.Commands.get(CommandArg[0]) || client.Commands.get(client.Aliases.get(CommandArg[0]));
